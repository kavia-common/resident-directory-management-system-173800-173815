...
export default function HouseholdClientPage({
  ...
  /** Resident ID used to look up the household unit. */
  residentId: string;
}) {
  const [state, setState] = useState<LoadState>({ kind: "idle" });

  const members = useMemo(
    () => (state.kind === "loaded" ? state.household.members : []),
    [state]
  );
  ...
  const load = async () => {
    ...
    try {
      const data = await apiGetHousehold(residentId);
      ...
      <section className="card">
        <div className="cardHeader">
          ...
          <span className="badge">
            {state.kind === "loaded" ? `Unit ${unit}` : "Unit —"}
          </span>
        </div>
